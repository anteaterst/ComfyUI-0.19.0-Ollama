14-04-2026 (frontend compatibility patch):

- Added `system_in` / `prompt_in` optional STRING input sockets to `OllamaGenerateV2` and `OllamaChat`
- Added `prompt_in` to deprecated `OllamaGenerate` and `prompt_in` / `system_in` to deprecated `OllamaGenerateAdvance`
- When connected, these sockets override the respective `system` / `prompt` widget values
- Works around the new ComfyUI frontend (v1.16+) widget→input conversion issues that prevented STRING outputs from being received correctly
- Added `dynamicPrompts: False` to the multiline text widgets to avoid unintended text transformations
