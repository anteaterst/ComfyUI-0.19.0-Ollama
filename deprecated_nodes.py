...
class OllamaGenerate:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                "prompt_in": ("STRING", {"forceInput": True, "tooltip": "Optional external prompt input. Overrides the 'prompt' widget when connected."}),
            },
        }
    ...
    def ollama_generate(self, prompt, debug, url, model, keep_alive, format, filter_thinking, prompt_in=None):

        if prompt_in is not None:
            if isinstance(prompt_in, list):
                prompt_in = prompt_in[0] if prompt_in else None
            if prompt_in is not None and str(prompt_in) != "":
...
class OllamaGenerateAdvance:
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                ...
                "prompt_in": ("STRING", {"forceInput": True, "tooltip": "Optional external prompt input. Overrides the 'prompt' widget when connected."}),
                "system_in": ("STRING", {"forceInput": True, "tooltip": "Optional external system prompt input. Overrides the 'system' widget when connected."}),
            }
        }
    ...
    def ollama_generate_advance(self, prompt, debug, url, model, system, seed, top_k, top_p, temperature, num_predict,
                                tfs_z, keep_alive, keep_context, format, filter_thinking, context=None, prompt_in=None, system_in=None):

        def _coalesce(external, fallback):
            if external is None:
                return fallback
        ...
        prompt = _coalesce(prompt_in, prompt)
        system = _coalesce(system_in, system)
