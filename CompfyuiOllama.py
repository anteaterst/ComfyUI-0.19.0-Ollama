...
class OllamaGenerateV2:
        ...
        return {
            "required": {
                "system": ("STRING", {
                    ...
                    "tooltip": "System prompt - use this to set the role and general behavior of the model. Can be overridden by connecting a STRING source to the 'system_in' socket."
                }),
                "prompt": ("STRING", {
                    "multiline": True,
                    "default": "What is art?",
                    "dynamicPrompts": False,
                    "tooltip": "User prompt - a question or task you want the model to answer or perform. Can be overridden by connecting a STRING source to the 'prompt_in' socket."
                }),
            ...
            "optional": {
                "system_in": ("STRING", {"forceInput": True, "tooltip": "Optional external system prompt input. If connected, overrides the 'system' widget value."}),
                "prompt_in": ("STRING", {"forceInput": True, "tooltip": "Optional external user prompt input. If connected, overrides the 'prompt' widget value."}),
    ...
    def ollama_generate_v2(self, system, prompt, think, keep_context, format, context = None, options=None, connectivity=None, images=None, meta=None, system_in=None, prompt_in=None):

        # Override system/prompt with the external socket values when connected.
        # Accept lists (ComfyUI sometimes wraps linked STRING values) and coerce to str.
        def _coalesce_text(external, fallback):
            if external is None:
                return fallback
        ...
        system = _coalesce_text(system_in, system)
        prompt = _coalesce_text(prompt_in, prompt)
...
class OllamaChat:
        ...
        return {
            ...
            "optional": {
                "system_in": (
                    ...
                    {
                        "forceInput": True,
                        "tooltip": "Optional external system prompt input. If connected, overrides the 'system' widget value.",
                    },
                ),
                "prompt_in": (
                    "STRING",
                    {
                        "forceInput": True,
                        "tooltip": "Optional external user prompt input. If connected, overrides the 'prompt' widget value.",
                    },
                ),
                "connectivity": (
                    "OLLAMA_CONNECTIVITY",
    ...
    def ollama_chat(
        ...
        system_in: str | list | None = None,
        prompt_in: str | list | None = None,
